...
export function KortGuideFilm({ film }: KortGuideFilmProps) {
  ...
  return (
    <article className="flex h-full min-h-[26rem] flex-col overflow-hidden rounded-2xl border border-border bg-surface shadow-sm">
      <div className="flex min-h-[7.5rem] flex-col border-b border-border bg-gradient-to-br from-[#eef6f0] to-[#e2f0e6] px-4 py-3 sm:px-5">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <span className="rounded-full bg-white px-2.5 py-0.5 text-xs font-medium text-primary-dark shadow-sm">
            {film.modul}
          </span>
      ...
      <div className="flex min-h-0 flex-1 flex-col p-4 sm:p-5">
        <div className="flex min-h-[14rem] flex-1 flex-col">
          {film.videoUrl ? (
            <video
              className="aspect-video h-full w-full rounded-xl border border-border bg-black object-cover"
              controls
              playsInline
              preload="metadata"
              src={film.videoUrl}
