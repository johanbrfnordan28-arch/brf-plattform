...
import { TakterrassPrisPanel } from "@/components/underhallsplan/TakterrassPrisPanel";
import { summeraTakterrassMangder } from "@/components/underhallsplan/takterrass-pris";
import {
  ...
  type TakterrassData,
  type TakterrassGolvMaterialId,
  type TakterrassTatskiktId,
  type TakterrassVaggMaterialId,
} from "@/components/underhallsplan/takterrass";

type TakterrassPanelProps = {
  data: TakterrassData;
...
function MaterialVal({
  ...
  /** Id för alternativet som visar fritext (t.ex. eget eller annat) */
  egetValId?: string;
}) {
...
export function TakterrassPanel({ data, onChange }: TakterrassPanelProps) {
  const d = data ?? skapaTomTakterrassData();

  function patch(p: Partial<TakterrassData>) {
    onChange({ ...d, ...p });
  }
  ...
  return (
    <div className="space-y-4">
      <p className="text-xs leading-relaxed text-muted">
        Gemensam takterrass — väggar, golv, tätskikt utomhus, avvattning och
        golvsockel. Fler material kan läggas till i listorna vid behov.
      </p>

      <MaterialVal
        legend="Väggar / uppkragningar"
        name="takterrass-vagg"
      ...
      <p className="text-xs text-muted">
        En normal gemensam takterrass är ofta cirka 20 m².
      </p>
      ...
      <fieldset className="rounded-lg border border-border bg-white p-3">
        ...
        <div className="mt-2 grid gap-3 sm:grid-cols-2">
          <MattFalt
            label="Belysning"
            enhet="st"
            value={d.belysningAntal}
            onChange={(belysningAntal) => patch({ belysningAntal })}
            step={1}
            placeholder="t.ex. 4"
          />
          <MattFalt
            label="Elkontakter"
            enhet="st"
            value={d.elkontaktAntal}
            onChange={(elkontaktAntal) => patch({ elkontaktAntal })}
            step={1}
            placeholder="t.ex. 2"
          />
        </div>
      ...
      <fieldset className="rounded-lg border border-border bg-white p-3">
        ...
        <p className="mt-2 text-xs text-muted">
          Ange kvm där golvvärme finns — lämna tomt om terrassen saknar golvvärme.
        </p>
      </fieldset>
      ...
      {(() => {
        const mangdRader = summeraTakterrassMangder(d);
      ...
      <TakterrassPrisPanel data={d} onChange={onChange} />
    </div>
  );
}
