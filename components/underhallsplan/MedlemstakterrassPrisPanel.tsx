...
import {
  ...
  medlemstakterrassPrisRadDef,
  type MedlemstakterrassPrisFaltId,
...
type MedlemstakterrassPrisPanelProps = {
  data: MedlemstakterrassData;
  onChange: (data: MedlemstakterrassData) => void;
};

export function MedlemstakterrassPrisPanel({
  data,
  onChange,
}: MedlemstakterrassPrisPanelProps) {
  const priser = data.priser ?? skapaTomMedlemstakterrassPriser();
  const { rader, totaltKr } = beraknaMedlemstakterrassPris(data);

  function uppdateraPris(id: MedlemstakterrassPrisFaltId, värde: string) {
    onChange({
      ...data,
  ...
  return (
    <fieldset className="rounded-lg border border-primary/30 bg-[#eef6f0]/50 p-3 sm:p-4">
      ...
      <p className="mt-1 text-xs leading-relaxed text-muted">
        Enhetspris per del — utan el. Summan bygger på mängderna ovan.
      </p>
      ...
      <div className="mt-3 overflow-x-auto rounded-lg border border-border bg-white">
        <table className="w-full min-w-[520px] text-left text-sm">
          ...
          <tbody>
            {medlemstakterrassPrisRadDef.map((def) => {
